...
5. **Verify the URDF** by running the internal `check_kinematic_chain.py` tool. You can pass your URDF path to see if your chain differs physically or structurally from other officially tested kinematic chains:
   ```bash
   python3 check_kinematic_chain.py ./urdfs/<your_generated_urdf>.urdf
   ```
6. Launch your teleop session by explicitly overriding the default URDF path in the arguments:
   ```bash
   python3 stretch_gamepad_teleop_gripper.py --urdf ./urdfs/<your_generated_urdf>.urdf
   ```

**Important Notes for Calder and Dali Robots:**
*   By default, `stretch_gamepad_teleop_gripper.py` points to `./urdfs/calder_ik.urdf`. This repository has been successfully tested and works well with both the Calder and Dali versions of Stretch 4.
